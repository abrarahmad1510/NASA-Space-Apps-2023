...
expInfo = {u'gender':u'', u'age': u'', u'participant number': u''}
        ...
        if block_order == 1:
            ...
            if blocks.thisN <= 3:
                ...
                if trial_type == 1: #flowers
                    ...
                    feedbackAllowed = "i"
                    feedbackCorrect = "i"
        widthRatio = win.size[1] / win.size[0]
        tempImage = Image.open(image_stimulus)
        width = tempImage.width
        height = tempImage.height
        maxDim = max(width, height)

        stimulusImageBox.setSize([(width/maxDim) * widthRatio * 0.9, (height/maxDim) * 0.9])
